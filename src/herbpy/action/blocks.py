    ...
    try:
        with AllDisabled(env, [table] + blocks, padding_only=True):
            ...
            with env:
                ...
                #current_finger_height = manip.GetEndEffectorTransform()[
                #    2, 3] - 0.14
        ...
        with AllDisabled(env, blocks + [table]):
            manip.PlanToEndEffectorOffset(
                direction=[0, 0, 1], distance=0.05, timelimit=5, execute=True)

        # OpenRAVE trick to hallucinate the block into the correct pose
        ...
        logger.error('Failed to complete block grasp: {}'.format(e))
        raise
...
def PlaceBlock(robot, block, on_obj, manip=None, **kw_args):
    ...
    with env:
        ...
        place_tsr_list = robot.tsrlibrary(
            block,
            'place_on',
