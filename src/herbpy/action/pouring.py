import openravepy, time, math, numpy
...
def MoveCupAndPour(robot, table, manip_pitcher, manip_cup, cup, pitcher):
    ...
    with Disabled(table):
        manip_cup.PlanToEndEffectorOffset(
            direction=[move_x, move_y, 0],
            distance=move_cup,
        ...
        with env:
            robot.Release(cup)
        manip_cup.PlanToEndEffectorOffset(
            direction=-1 * manip_cup_pose[:3, 2],
            distance=0.1,
            position_tolerance=0.1,
            execute=True,
            timelimit=10)
    ...
    manip_pitcher.PlanToEndEffectorOffset(
        direction=-1 * manip_pitcher_pose[:3, 0],
        distance=0.1,
        position_tolerance=0.1,
        execute=True)
