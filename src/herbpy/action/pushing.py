...
def PushToPoseOnTable(robot,
    ...
    @param goal_position The desired (x,y) position of the object in
                         world coordinates
    @param goal_radius The max distance from goal_position for the object to
                       be to still consider the goal achieved
    ...
    @param tracker The perception module to use to track the object during
                   pushing
    """
    # Get a push planner
    try:
        ...
        raise ActionError(
            "Unable to create PushPlanner. Is the"
        ...
        with RenderTrajectory(robot, traj, color=[0, 0, 1, 1], render=render):
            ...
            try:
                if manip.simulated:
                    ...
                    # expected poses at the end of the trajectory. If execution
                    # was successful, this should resolve collisions. 
