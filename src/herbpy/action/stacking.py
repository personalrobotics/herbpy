...
def StackCups(robot, table, cup, stack, cups_stacked, manip=None):
    ...
    with Disabled(table):
        ...
        manip.PlanToEndEffectorOffset(direction=[0, 0, 1],
               distance=stack_height, position_tolerance=0.1, execute=True)
    ...
    manip.PlanToEndEffectorOffset(direction=direc, distance=dist,
              position_tolerance=0.05, execute=True)

    #Move cup down
    with Disabled(stack):
        manip.PlanToEndEffectorOffset(direction=[0, 0, -1], distance=0.02,
              position_tolerance=0.1, execute=True)
