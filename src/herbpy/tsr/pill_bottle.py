...
def pills_push_grasp(robot, pills, manip=None, push_distance=0.1, **kw_args):
    ...
    '''
    return _pills_grasp(
        robot, pills, manip=manip, push_distance=push_distance, **kw_args)
...
def _pills_grasp(robot, pills, manip=None, push_distance=0.0, **kw_args):
    ...
    grasp_chain = TSRChain(
        sample_start=False, sample_goal=True, constrain=False, TSR=grasp_tsr)
...
def pills_on_table(robot, pills, pose_tsr_chain, manip=None):
    ...
    ee_in_glass = numpy.dot(
        numpy.linalg.inv(pills.GetTransform()),
...
def pills_transport(robot,
    ...
    little roll, pitch or yaw. Assumes the object has already been grasped and
    is in the proper configuration for transport.

    @param robot The robot grasping the glass
    @param pills The grasped object
    @param manip The manipulator grasping the object, if None the active
                 manipulator of the robot is used
    @param roll_epsilon The amount to let the object roll
                        during transport (object frame)
    @param pitch_epsilon The amount to let the object pitch
                         during transport (object frame)
    @param yaw_epsilon The amount to let the object yaw
                       during transport (object frame)
    '''
